...
let command =
  ...
  ~readme:(fun () -> "Given a file with a hitbox stream channel name on each line, the program determines which ones are currently live")
  spec
  (fun filename () -> 
    Async.Std.Reader.file_lines filename >>= fun streams ->
    Hitbox_api.get_live_streams streams >>| fun live_streams ->
